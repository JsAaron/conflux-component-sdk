<template>
	<view class="u-demo">
		<view>
			<u-alert-tips type="primary" title="省市区的数据源更改成了 uniCloud 中 opendb-city-china 表内的数据源，方便与云函数对接。且返回值也与数据库对齐: 属性 value 改成了 code, label 改成了 name"></u-alert-tips>
		</view>
		<view class="u-demo-wrap mt10">
	...
	methods: {
		...
		confirm(e) {
			...
			if (this.mode == 'time') {
				...
				if (e.province.name) {
					this.input = e.province.name + '-' + e.city.name + '-' + e.area.name;
				} else {
